...
from parse_title import parse, parse_daytimes
...
async def read_forum(guild, key, sessions, unparsed, processed_urls, today, prev_by_url=None):
    ...
    for th in ths:
        ...
        # 複数日で日ごとに時刻が違う卓＝本文から日別時刻を拾う（生本文は保存しない・抽出結果だけ）。
        # 拾えた日だけ上書き、それ以外はタイトル既定の時刻にフォールバック。
        per_day = parse_daytimes(r["dates"], starter.content if starter else "") if len(r["dates"]) >= 2 else {}
        def _mkdate(datestr, m, d):
            # 本文から拾えた日だけ時刻を上書き＆time_sourceを付与。それ以外はタイトル既定＝従来と完全一致（退行ゼロ）。
            if (m, d) in per_day:
                st, en = per_day[(m, d)]
                return {"date": datestr, "start": st, "end": en, "time_source": "body"}
            return {"date": datestr, "start": r["start"], "end": r["end"]}
        dates = []
        for (m, d) in r["dates"]:
            y = infer_year(m, d, base_dt)
            if y:
                dates.append(_mkdate(f"{y}-{m:02d}-{d:02d}", m, d))
            ...
            for (m, d) in r["dates"]:
                ...
                try:
                    if datetime.date(yy, m, d) < today:
                        yy += 1
                    rolled.append(_mkdate(f"{yy}-{m:02d}-{d:02d}", m, d))
                except ValueError:
                    pass
            if rolled:
                dates = rolled
